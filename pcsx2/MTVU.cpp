...
void VU_Thread::Reset()
{
	...
	memzero(vifRegs);
	for (size_t i = 0; i < 4; ++i)
		vu1Thread.vuCycles[i].store(0, std::memory_order_release);
}
		...
		while (read_pos.load(std::memory_order_relaxed) != GetWritePos()) {
			...
			switch (tag) {
				case MTVU_VU_EXECUTE: {
					...
					// Greg: warning AtomicExchange will generate a barrier
...
void VU_Thread::KickStart(bool forceKick)
{
	...
	|| (!isBusy && GetReadPos() != write_pos.load(std::memory_order_relaxed))) semaEvent.Post();
}
